...
var author$project$CollatzConjecture$makeSteps = F2(
	function (value, count) {
		makeSteps:
		while (true) {
			if (value === 1) {
				return _Utils_Tuple2(value, count);
			} else {
				var rest = A2(elm$core$Basics$modBy, 2, value);
				if (rest === 1) {
					var $temp$value = (value * 3) + 1,
						$temp$count = count + 1;
					value = $temp$value;
					count = $temp$count;
					continue makeSteps;
				} else {
					var $temp$value = (value / 2) | 0,
						$temp$count = count + 1;
					value = $temp$value;
					count = $temp$count;
					continue makeSteps;
...
var author$project$CollatzConjecture$collatz = function (start) {
	if (start <= 0) {
		return elm$core$Result$Err('Only positive numbers are allowed');
	} else {
		var _n0 = A2(author$project$CollatzConjecture$makeSteps, start, 0);
		var count = _n0.b;
...
var author$project$Test$Generated$Main3491402838$main = A2(
	author$project$Test$Runner$Node$run,
	{
		paths: _List_fromArray(
			['/home/ghibulo/exercism2/elm/collatz-conjecture/tests/Tests.elm']),
		processes: 4,
		report: author$project$Test$Reporter$Reporter$ConsoleReport(author$project$Console$Text$UseColor),
		runs: elm$core$Maybe$Nothing,
		seed: 114146708300390
	},
...
_Platform_export({'Test':{'Generated':{'Main3491402838':{'init':author$project$Test$Generated$Main3491402838$main(elm$json$Json$Decode$int)(0)}}}});}(this));
return this.Elm;
})({});
var pipeFilename = "/tmp/elm_test-6692.sock";
